	...
	{:else}
		<form class="card m-2 flex flex-col items-center gap-2 rounded border p-2 sm:p-6" method="post" action="?/OAuth" onsubmit={handleSubmit}>
			...
			<div class="mt-2 flex w-full justify-between gap-1 sm:gap-2">
				<!-- Cancel Button -->
				<button type="button" onclick={handleCancel} aria-label={m.button_cancel()} class="btn preset-filled">
					{m.button_cancel()}
				</button>

				<!-- Submit Button -->
				<button type="submit" disabled={!isFormValid || isLoading} aria-label={m.button_send()} class="btn items-center preset-filled">
